	...
	u32 bl_lvl)
{
	...
	if (mi_get_panel_id(panel->mi_cfg.mi_panel_id) == M2_PANEL_PA) {
          	if (mi_cfg->feature_val[DISP_FEATURE_HBM] == FEATURE_ON && bl_lvl == 0) {
			mi_dsi_update_51_mipi_cmd(panel, DSI_CMD_SET_MI_HBM_OFF, 0);
			dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_HBM_OFF);
			mi_cfg->feature_val[DISP_FEATURE_HBM] = FEATURE_OFF;
		}
		if (bl_lvl >= 329 && bl_lvl <= 331)
			bl_lvl = 332;
		else if (bl_lvl >= 326 && bl_lvl <= 328)
			bl_lvl = 325;
	}

	rc = dsi_panel_send_em_cycle_setting(panel, bl_lvl);
	if (rc < 0)
		DSI_ERR("failed to send em cycle: %d\n", bl_lvl);

	if (mi_get_panel_id(mi_cfg->mi_panel_id) == N11_PANEL_PA) {
		rc = dsi_panel_send_em_cycle_setting_N11(panel, bl_lvl);
		if (rc < 0)
			DSI_ERR("failed to send em cycle: %d\n", bl_lvl);
	}
	if (mi_get_panel_id_by_dsi_panel(panel) == N11_PANEL_PA) {
		if (bl_lvl >= PEAK_HDR_BL_LEVEL && bl_lvl < MAX_BL_LEVEL && !panel->mi_cfg.is_peak_hdr) {
			...
			DSI_INFO("bl_lvl is %d,peakhdr mode on,flat mode off\n", bl_lvl);
		}

		if (bl_lvl != 0 && bl_lvl < PEAK_HDR_BL_LEVEL && panel->mi_cfg.is_peak_hdr) {
			if (panel->mi_cfg.feature_val[DISP_FEATURE_FLAT_MODE]) {
			...
			DSI_INFO("bl_lvl is %d,exit peakhdr mode on,flat mode on\n", bl_lvl);
		}
	}
	...
	if ((mi_get_panel_id(mi_cfg->mi_panel_id) == M11_PANEL_PA) ||
		(mi_get_panel_id(mi_cfg->mi_panel_id) == N11_PANEL_PA)) {
		mi_dsi_panel_set_gamma_update_reg(panel, bl_lvl);
	}

	if (bl_lvl !=0 && mi_get_panel_id(mi_cfg->mi_panel_id) == M2_PANEL_PA && panel->qsync_enable) {
		DSI_INFO("qsync is enabled, do not allow set backlight!");
		goto skip_setbl;
	}

	if (mi_get_panel_id(mi_cfg->mi_panel_id) == M1_PANEL_PA) {
	...
	if (mi_get_panel_id(mi_cfg->mi_panel_id) == M1_PANEL_PA) {
		dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_FORCE_UPDARE_FRAME);
	}
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	"mi,mdss-dsi-panel-wp-read-offset-command",
	...
	"mi,mdss-dsi-dbi-bwg-500-1300nit-mode-command",
	...
	"mi,mdss-dsi-skip-source-500-1300nit-mode-command",
	...
	"mi,mdss-dsi-skip-source-0-30nit-mode-command",
	"mi,mdss-dsi-force-update-frame-command",
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"mi,mdss-dsi-panel-wp-read-offset-command-state",
	...
	"mi,mdss-dsi-dbi-bwg-500-1300nit-mode-command-state",
	...
	"mi,mdss-dsi-dbi-bwg-0-53nit-mode-command-state",
	...
	"mi,mdss-dsi-skip-source-500-1300nit-mode-command-state",
	...
	"mi,mdss-dsi-skip-source-0-53nit-mode-command-state",
	"mi,mdss-dsi-force-update-frame-command-state",
	...
	"mi,mdss-dsi-temperature-code-command-state",
	"mi,mdss-dsi-temperature-lut1-command-state",
	"mi,mdss-dsi-temperature-lut2-command-state",
...
const char *cmd_set_update_map[DSI_CMD_UPDATE_MAX] = {
	...
	"qcom,mdss-dsi-post-mode-switch-on-command-update",
	...
	"mi,mdss-dsi-local-hbm-off-to-normal-em-cycle-16pulse-command-update",
	...
	"mi,mdss-dsi-local-hbm-off-to-hlpm-command-update",
	"mi,mdss-dsi-local-hbm-off-to-llpm-command-update",
		...
		int ctrl_idx)
{
	...
	mi_cfg = &panel->mi_cfg;
        if (mi_get_panel_id(mi_cfg->mi_panel_id) == M2_PANEL_PA)
                mi_dsi_update_em_cycle_cmd(panel, DSI_CMD_SET_QSYNC_ON, mi_cfg->last_bl_level);
		...
		int ctrl_idx)
{
	...
	mi_cfg = &panel->mi_cfg;
	if (mi_get_panel_id(mi_cfg->mi_panel_id) == M2_PANEL_PA)
		mi_dsi_update_em_cycle_cmd(panel, DSI_CMD_SET_QSYNC_OFF, mi_cfg->last_bl_level);
...
int dsi_panel_switch_locked(struct dsi_panel *panel)
{
	int rc = 0;
	u32 fps_mode;
	...
	/* Need check panel lock is locked！*/
	if (WARN_ON(!mutex_is_locked(&panel->panel_lock)))
		return -EINVAL;
	...
	if (mi_get_panel_id(mi_cfg->mi_panel_id) == N11_PANEL_PA) {
		mi_dsi_update_switch_cmd_N11(panel);
	}

	if (mi_get_panel_id(mi_cfg->mi_panel_id) == M1_PANEL_PA) {
		mi_dsi_update_timing_switch_and_flat_mode_cmd(panel, DSI_CMD_SET_TIMING_SWITCH);
		if (((panel->cur_mode->timing.h_skew >> FPS_SF_FPS_OFFSET) & FPS_VALUE_MASK) != 1) {
	...
	if (mi_get_panel_id(mi_cfg->mi_panel_id) == M2_PANEL_PA) {
		mi_dsi_update_em_cycle_cmd(panel,DSI_CMD_SET_TIMING_SWITCH,mi_cfg->last_bl_level);
	...
	if (mi_get_panel_id(mi_cfg->mi_panel_id) == N11_PANEL_PA) {
		...
		if (cur_refresh_rate == 90 && mi_cfg->last_refresh_rate != 90) {
			DSI_INFO("panel switch 90hz disable lut setting\n");
			mi_dsi_panel_set_3dlut_temperature_compensation(panel, 0x80, false);
		} else if ( (cur_refresh_rate == 60 || cur_refresh_rate ==120) &&
				mi_cfg->last_refresh_rate == 90) {
			DSI_INFO("panel switch 60/120 hz restore lut setting\n");
			mi_dsi_panel_set_3dlut_temperature_compensation(panel, mi_cfg->feature_val[DISP_FEATURE_DBI], false);
		}
	}
	...
	if(panel->cur_mode->timing.h_skew == FPS_NORMAL)
		...
		mi_cfg->last_refresh_rate = panel->cur_mode->timing.h_skew & FPS_VALUE_MASK;
	else if ((fps_mode & FPS_MODE_AUTO) || (fps_mode & FPS_MODE_QSYNC))
		mi_cfg->last_refresh_rate = (panel->cur_mode->timing.h_skew >> FPS_SF_FPS_OFFSET) & FPS_VALUE_MASK;
...
int dsi_panel_switch(struct dsi_panel *panel)
{
	...
	rc = dsi_panel_switch_locked(panel);
...
int dsi_panel_post_switch(struct dsi_panel *panel)
{
	...
	if(mi_cfg->last_refresh_rate == 1){
		mi_dsi_update_timing_switch_and_flat_mode_cmd(panel,DSI_CMD_SET_POST_TIMING_SWITCH);
		mi_cfg->dbi_bwg_type = DSI_CMD_SET_MAX;
		mi_dsi_panel_vrr_set_by_dbv(panel, mi_cfg->last_bl_level);
	}
...
int dsi_panel_enable(struct dsi_panel *panel)
{
	...
	if (mi_get_panel_id_by_dsi_panel(panel) == N11_PANEL_PA) {
		mi_dsi_panel_cal_update_peak_hdr_gamma_N11(panel);
		if (gamma_cfg->update_done)
	...
	panel->mi_cfg.skip_source_type = DSI_CMD_SET_MAX;
	...
	if (panel->mi_cfg.dbi_3dlut_compensate_enable) {
		mi_dsi_panel_set_3dlut_temperature_compensation(panel,
			panel->mi_cfg.feature_val[DISP_FEATURE_DBI], true);
	}
...
int dsi_panel_disable(struct dsi_panel *panel)
{
	...
	if (mi_cfg->feature_val[DISP_FEATURE_PEAK_HDR_MODE]){
