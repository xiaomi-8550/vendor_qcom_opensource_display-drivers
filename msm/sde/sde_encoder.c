			...
			u32 *qsync_fps, struct drm_connector_state *conn_state)
{
	...
	*qsync_fps = 0;
	...
	if (sde_conn->qsync_min_fps_index) {
		*qsync_fps = sde_conn->qsync_min_fps_index;
	} else {
		...
		bool force_update_dsi_clocks)
{
	...
	if (sde_enc->cur_master && sde_enc->cur_master->connector) {
		...
		rc = sde_connector_pre_kickoff(conn_mas, force_update_dsi_clocks);
		if (rc) {
		...
		struct drm_crtc_state *old_crtc_state)
{
	...
	ret = _sde_encoder_prepare_for_kickoff_processing(drm_enc, params, sde_enc, sde_kms,
			needs_hw_reset, is_cmd_mode, force_update_dsi_clocks);
end:
...
void sde_encoder_kickoff(struct drm_encoder *drm_enc, bool config_changed)
{
	...
	if (dsi_display && dsi_display->panel && (mi_get_panel_id(dsi_display->panel->mi_cfg.mi_panel_id) == N81A_PANEL_PA)&&
	...
	if (dsi_display && dsi_display->panel && (mi_get_panel_id(dsi_display->panel->mi_cfg.mi_panel_id) == N81A_PANEL_PA)
		...
		if (rc) {
				pr_err("Failed to send DSI_CMD_SET_TIMING_VIDEO_SWITCH command\n");
			}
